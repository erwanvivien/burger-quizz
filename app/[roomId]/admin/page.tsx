...
function AdminPanel({ roomId }: { roomId: string }) {
  ...
  function handleAddTeam() {
    ...
    const availableColor =
      TEAM_COLORS.find((c) => !usedColors.includes(c)) || "red";
    setNewTeamName(
      DEFAULT_TEAM_NAMES[state.teams.length] ||
        `Équipe ${state.teams.length + 1}`
    );
  ...
  function handleRenameConfirm() {
    ...
    if (name)
      socketRef.current?.emit("team:rename", {
        roomId,
    ...
    >
      ...
      >
        ...
        <span style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
          Salle :{" "}
          <strong style={{ color: "var(--secondary)" }}>{roomId}</strong>
        </span>
        <span
          style={{
            marginLeft: "auto",
          ...
          {state.teams.map((team) => (
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() =>
                    socketRef.current?.emit("team:remove", {
                      roomId,
                      teamId: team.id,
                    })
                  }
              ...
              >
                <button
                  onClick={() =>
                    socketRef.current?.emit("score:adjust", {
                      roomId,
                      teamId: team.id,
                      delta: -1,
                    })
          ...
          (addingTeam ? (
            ...
            >
              ...
              <div style={{ display: "flex", gap: "0.4rem" }}>
                {TEAM_COLORS.filter((c) => !usedColors.includes(c)).map(
                  (color) => (
                    <button
                      key={color}
                      ...
                      style={{
                        ...
                        borderRadius: "0.5rem",
                        border: `3px solid ${
                          newTeamColor === color ? "#fff" : "transparent"
                        }`,
          ...
          {state.phase === "lobby" && (
            <button
              ...
              style={{
                ...
                flex: 1,
                background:
                  state.teams.length >= 2 ? "var(--success)" : "var(--surface)",
              ...
              {[...state.teams].map((team, i) => (
                <div
                  ...
                  style={{
                    ...
                    background:
                      i === 0
                        ? `${COLOR_HEX[team.color]}33`
                        : "var(--surface2)",
                    border: `1px solid ${
                      i === 0 ? COLOR_HEX[team.color] : "transparent"
                    }`,
                  }}
                >
